...
def evaluate(eval_cfg, mode):
    ...
    for file in os.listdir(eval_cfg["val_result_path"]):
        ...
        evaluate_on_coco_caption(
            file_path, val_caption_coco_file_path, eval_result_path
        )
        evaluate_summary(
            file_path,
    ...
    # evaluate_summary(file_path, test_summary_anno_path, test_eval_result_path, fps, sum_ratio, mode)


#  this is a script for testing evaluation result
if __name__ == "__main__":
    eval_cfg = dict()
    eval_cfg["fps"] = 8
    eval_cfg["sum_ratio"] = 0.15
    eval_cfg["val_caption_coco_file_path"] = (
        "data/caption_coco_format/val.caption_coco_format.json"
    )
    eval_cfg["test_caption_coco_file_path"] = (
        "data/caption_coco_format/test.caption_coco_format.json"
    )
    eval_cfg["val_summary_anno_path"] = "data/summary_annotation/val.jsonl"
    eval_cfg["test_summary_anno_path"] = "data/summary_annotation/test.jsonl"
    eval_cfg["val_result_path"] = "multi-run/seed99/ubasis_swin_ndcg_warm30/val_result"
    eval_cfg["eval_result_path"] = (
        "multi-run/seed99/ubasis_swin_ndcg_warm30/eval_result"
    )
    eval_cfg["test_result_path"] = ""
    eval_cfg["save_val_result"], eval_cfg["save_eval_result"] = True, True
    eval_cfg["num_frames"] = 80
    evaluate(eval_cfg, "2s")
